...
class Order extends CI_Model 
{
    ...
    public function getTotalExtrasBySucursalBetweenDatesByExtra($fechaDesde,$fechaHasta,$idSucursal,$idExtra){
        ...
        $where = "oe.order_id in (select orders.id from orders where orders.office_id  = $idSucursal AND orders.created_at >= '$fechaDesde 00:00:00' AND orders.created_at <= '$fechaHasta 23:59:59' AND orders.deliver_type = 'SUC'  AND orders.id_estado_pedido not in (4)) AND oe.extra_id = $idExtra";
        $this->db->where($where);        
    ...
    public function getTotalExtrasBySucursalByDiaBolsonByExtra($diaBolson,$idSucursal,$idExtra){
        ...
        $this->db->select('oe.id, oe.cant');
        $this->db->from('orders_extras as oe');
        $where = "oe.order_id in (select orders.id from orders where orders.deliver_date = '$diaBolson' AND orders.office_id = $idSucursal AND orders.deliver_type = 'SUC' AND orders.id_estado_pedido not in (4)) AND oe.extra_id = $idExtra";
        $this->db->where($where);        
        $extras = $this->db->get()->result();
        $contExtra = 0;
        if(!empty($extras)){
    ...
    public function getTotalExtraByDiaBolsonByExtra($diaBolson,$idExtra){
        //FUNCION QUE SE USA PARA EL EXPORT A EXCEL DE LOS PEDIDOS ENTRE LAS FECHAS DE SUCURSAL. 
        //ME DA EL TOTALIZADO POR EXTRA POR DIA BOLSON
        $this->db->select('oe.id, oe.cant');
        $this->db->from('orders_extras as oe');
        $where = "oe.order_id in (select orders.id from orders where orders.deliver_date = '$diaBolson' AND orders.deliver_type = 'SUC') AND oe.extra_id = $idExtra";
        $this->db->where($where);        
        $extras = $this->db->get()->result();
        $contExtra = 0;
        if(!empty($extras)){
    ...
    public function getTotalExtraBetweenDatesByExtra($fechaDesde,$fechaHasta,$idExtra){
        ...
        $this->db->select('oe.id, oe.cant');
        $this->db->from('orders_extras as oe');
        $where = "oe.order_id in (select orders.id from orders where orders.created_at >= '$fechaDesde 00:00:00' AND orders.created_at <= '$fechaHasta 23:59:59' AND orders.deliver_type = 'SUC') AND oe.extra_id = $idExtra";
        $this->db->where($where);        
        $extras = $this->db->get()->result();
        $contExtra = 0;
        if(!empty($extras)){
    ...
    public function generateOrdersArray($orders){
        ...
        foreach ($orders as $order){
            ...
            array_push($toAppend, array(
                ...
                'cantidad_bolsones' => $order->cantidad_bolsones,
                ...
                'monto_debe' => ($order->monto_total - $order->monto_pagado),
    ...
    public function getOrdersSucursalWithExtrasBetweenDates($fechaDesde,$fechaHasta){
        ...
        $where = "deliver_type = 'SUC' AND created_at >= '$fechaDesde 00:00:00' AND created_at <= '$fechaHasta 23:59:59' AND id_estado_pedido not in (4)";
        $this->db->where($where);
    ...
    public function getOrdersSucursalWithExtrasByDiaDeBolson($diaBolson){
        ...
        //Excluyo los pedidos en estado CANCELADO
        $where = "deliver_type = 'SUC' AND deliver_date = '$diaBolson' AND id_estado_pedido not in (4)";    
    ...
    public function getOrdersADomicilioWithExtrasByDiaDeBolson($diaBolson){
        ...
        $this->db->select('orders.id, orders.client_name, orders.email, orders.phone, bolson.name as cantidad_bolsones, bolson.price as precio_bolson, bolson.cant as cant_bolson, bolson.id as id_bolson, orders.deliver_date, barrio.id as id_barrio, barrio.nombre as nombre_barrio, barrio.observaciones as barrio_observaciones, orders.deliver_address as cliente_domicilio, orders.deliver_extra as cliente_domicilio_extra, orders.created_at, orders.observaciones, orders.monto_total, orders.monto_pagado, orders.id_estado_pedido, orders.nro_orden');
        $this->db->from('orders');
    ...
    public function getOrdersADomicilioWithExtrasBetweenDates($fechaDesde,$fechaHasta){
        ...
        $this->db->join('barrios as barrio', 'barrio.id = orders.barrio_id', 'left');
        $this->db->join('pockets as bolson', 'bolson.id = orders.pocket_id', 'left');
        //$where = "orders.deliver_type = 'DEL' AND orders.valid = 1 AND orders.created_at >= '$fechaDesde 00:00:00' AND orders.created_at <= '$fechaHasta 23:59:59'";
        //Excluyo los pedidos en estado CANCELADO
    ...
    public function getTotalExtraADomicilioByDiaBolsonByExtra($diaBolson,$idExtra){
        //FUNCION QUE SE USA PARA EL EXPORT A EXCEL
        //ME DA EL TOTALIZADO POR EXTRA POR DIA BOLSON A DOMICILIO
        $this->db->select('oe.id, oe.cant');
        $this->db->from('orders_extras as oe');
        $where = "oe.order_id in (select orders.id from orders where orders.deliver_date = '$diaBolson' AND orders.deliver_type = 'DEL' AND orders.valid = 1) AND oe.extra_id = $idExtra";
        $this->db->where($where);        
        $extras = $this->db->get()->result();
        $contExtra = 0;
        if(!empty($extras)){
    ...
    public function getTotalExtraADomicilioBetweenDatesByExtra($fechaDesde,$fechaHasta,$idExtra){
        ...
        $this->db->select('oe.id, oe.cant');
        $this->db->from('orders_extras as oe');
        $where = "oe.order_id in (select orders.id from orders where orders.created_at >= '$fechaDesde 00:00:00' AND orders.created_at <= '$fechaHasta 23:59:59' AND orders.deliver_type = 'DEL' AND orders.valid = 1) AND oe.extra_id = $idExtra";
        $this->db->where($where);        
        $extras = $this->db->get()->result();
        $contExtra = 0;
        if(!empty($extras)){
    ...
    public function updatePedido($id, $nombre, $telefono, $mail, $direccion, $direccionPisoDepto, $idTipoPedido, $idBarrio, $idSucursal, $idBolson, $montoTotal, $montoPagado, $idEstadoPedido, $observaciones, $idPedidoFijo)
    {
    ...
    {
        $this->db->select('id, client_name, email, phone, office_id, pocket_id, deliver_date, created_at, deliver_type, deliver_address, deliver_extra, barrio_id, valid, hash, monto_total, monto_pagado, id_estado_pedido, observaciones, id_tipo_pedido, id_pedido_fijo, id_pedido_original');
        $this->db->from('orders');
        $this->db->order_by('created_at', 'DESC');
    ...
    {
        $query = $this->db->query("SELECT ep.*, oe.cant FROM extra_products ep LEFT JOIN orders_extras oe ON (ep.id = oe.extra_id) WHERE oe.order_id = " . $orderId);
        return $query->result();
    }
    ...
    public function getAllPedidosFijos(){
        $this->db->select('id, client_name, email, phone, office_id, pocket_id, deliver_date, created_at, deliver_type, deliver_address, deliver_extra, barrio_id, valid, hash, monto_total, monto_pagado, id_estado_pedido, observaciones, id_tipo_pedido, id_pedido_fijo');
        $this->db->from('orders');
        $this->db->order_by('created_at', 'DESC');
        $this->db->where('valid', 1);
    ...
    public function getPuntosRetiroInDiaEntrega($idDiaEntrega){
        ...
        $where = "office.id in (select orders.office_id from orders where orders.id_dia_entrega = '$idDiaEntrega' AND orders.deliver_type = 'SUC' AND orders.id_estado_pedido not in (4))";
        $this->db->where($where);        
    ...
    public function getTotalOrdersByIdDiaEntregaAndIdPuntoRetiro($idDiaBolson,$idPuntoRetiro){
        ...
        $this->db->join('offices as sucursal', 'sucursal.id = orders.office_id', 'left');
        $this->db->join('pockets as bolson', 'bolson.id = orders.pocket_id', 'left');
        $where = "(orders.id_estado_pedido not in (4) AND orders.id_dia_entrega = '$idDiaBolson' AND orders.office_id = $idPuntoRetiro AND orders.deliver_type = 'SUC')";        
        $this->db->where($where);        
        $orders = $this->db->get()->result();
        $contBolsones = 0;
        if(!empty($orders)){
    ...
    public function getBarriosInDiaEntrega($idDiaEntrega){
        ...
        $where = "barrio.id in (select orders.barrio_id from orders where orders.id_dia_entrega = '$idDiaEntrega' AND orders.deliver_type = 'DEL' AND orders.valid = 1 AND orders.id_estado_pedido not in (4))";
        $this->db->where($where);        
    ...
    public function getTotalOrdersEspecialesByIdDiaEntregaAndIdPuntoRetiro($idDiaBolson,$idPuntoRetiro){
        ...
        $this->db->join('offices as sucursal', 'sucursal.id = orders.office_id', 'left');
        $this->db->join('pockets as bolson', 'bolson.id = orders.pocket_id', 'left');
        $where = "(orders.id_estado_pedido in (2) AND orders.id_dia_entrega = '$idDiaBolson' AND orders.office_id = $idPuntoRetiro AND orders.deliver_type = 'SUC')";        
        $this->db->where($where);        
        $orders = $this->db->get()->result();
        $contBolsones = 0;
        if(!empty($orders)){
    ...
    public function getOrdersPuntosRetiroWithExtrasByIdDiaEntregaAndIdPuntoRetiro($idDiaEntrega, $idPuntoRetiro){
        //FUNCION QUE SE USA PARA EL EXPORT A EXCEL DE LOS PEDIDOS ENTRE LAS FECHAS DE SUCURSAL
        $this->db->select('orders.id, orders.client_name, orders.email, orders.phone, sucursal.id as id_sucursal, sucursal.name as sucursal, sucursal.address as domicilio_sucursal, bolson.name as cantidad_bolsones, bolson.price as precio_bolson, bolson.cant as cant_bolson, bolson.id as id_bolson, orders.deliver_date, orders.id_dia_entrega, orders.created_at, orders.observaciones, orders.monto_total, orders.monto_pagado, orders.id_estado_pedido, orders.nro_orden');
        $this->db->from('orders');
        $this->db->join('offices as sucursal', 'sucursal.id = orders.office_id', 'left');
        $this->db->join('pockets as bolson', 'bolson.id = orders.pocket_id', 'left');
        //$where = "deliver_type = 'SUC' AND deliver_date = '$diaBolson'";    
        //Excluyo los pedidos en estado CANCELADO
        $where = "orders.deliver_type = 'SUC' AND orders.id_dia_entrega = '$idDiaEntrega' AND orders.office_id = '$idPuntoRetiro' AND orders.id_estado_pedido not in (4)";    
    ...
    public function getTotalExtrasByPuntoDeRetiroByIdDiaEntregaByIdExtra($idPuntoDeRetiro,$idDiaEntrega,$idExtra){
        ...
        $this->db->select('oe.id, oe.cant');
        $this->db->from('orders_extras as oe');
        $where = "oe.order_id in (select orders.id from orders where orders.id_dia_entrega = '$idDiaEntrega' AND orders.office_id = $idPuntoDeRetiro AND orders.deliver_type = 'SUC' AND orders.id_estado_pedido not in (4)) AND oe.extra_id = $idExtra";
        $this->db->where($where);        
        $extras = $this->db->get()->result();
        $contExtra = 0;
        if(!empty($extras)){
    ...
    public function getOrdersBarriosWithExtrasByIdDiaEntregaAndIdBarrio($idDiaEntrega, $idBarrio){
        //FUNCION QUE SE USA PARA EL EXPORT A EXCEL DE LOS PEDIDOS ENTRE LAS FECHAS DE SUCURSAL
        $this->db->select('orders.id, orders.client_name, orders.email, orders.phone, barrio.id as id_barrio, barrio.nombre as nombre_barrio, barrio.observaciones as barrio_observaciones, orders.deliver_address as cliente_domicilio, orders.deliver_extra as cliente_domicilio_extra, bolson.name as cantidad_bolsones, bolson.price as precio_bolson, bolson.cant as cant_bolson, bolson.id as id_bolson, orders.deliver_date, orders.id_dia_entrega, orders.created_at, orders.observaciones, orders.monto_total, orders.monto_pagado, orders.id_estado_pedido, orders.nro_orden');
        $this->db->from('orders');
        $this->db->join('barrios as barrio', 'barrio.id = orders.barrio_id', 'left');
        $this->db->join('pockets as bolson', 'bolson.id = orders.pocket_id', 'left');
        //Excluyo los pedidos en estado CANCELADO
