...
class Main extends CI_Controller
{
        ...
        */
        ...
        if(count($errors) > 0) {
            ...
            if($montoTotalPedido>=$montoMinimoParaEnvioSinCargo){
                ...
                $montoDelivery = (int)$this->Parameter->get('costoEnvioPedidos');
            }
    ...
    function paymenConfirmation($hash) {
        ...
        $this->Order->savePaymentId($hash,$paymentId);
        
        $ch = curl_init('https://api.mercadopago.com/v1/payments/'.$paymentId);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_HTTPHEADER, array(
            'Authorization: Bearer APP_USR-7037265360358719-051716-2138eda82413bdb18b7fac33c97dfb3c-417720824'
        ));
        $result = curl_exec($ch);
        curl_close($ch);
        ...
        $fechaCreacion = $responseJson['date_created'];
        $fechaAprobacion = $responseJson['date_approved'];
        //error_log("\Fecha Aprobacion:\n".isset($fechaAprobacion)."\nFecha Creacion".isset($fechaCreacion),3,"../../logs/errorAgus.log");        
        
        if(isset($fechaAprobacion) && strlen($fechaAprobacion)>5) {
